...
class _PetFormDialogState extends State<PetFormDialog> {
  ...
  void initState() {
    ...
    _costController = TextEditingController(text: widget.pet?.estimatedCost.toString() ?? '0');
    _nextVaccination = widget.pet?.nextVaccination ?? DateTime.now().add(Duration(days: 365));
          ...
          onPressed: () {
            if (_formKey.currentState!.validate()) {
              final pet = Pet(
                ...
                lastVaccination: widget.pet?.lastVaccination ?? DateTime.now().subtract(Duration(days: 30)),
                nextVaccination: _nextVaccination,
                lastBath: widget.pet?.lastBath ?? DateTime.now().subtract(Duration(days: 7)),
                nextBath: _nextBath,
